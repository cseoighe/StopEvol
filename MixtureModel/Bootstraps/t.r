...
load(file='setup.out.RData')
load(file=paste('sumLstep', step,'.RData',sep=""))
load(file=paste('splinefstep', step,'.RData',sep="")) 
...
for(si in 1:length(stopcodonlist)){
  ...
  for(bi in 1:bootstrap_sample){
    ...
    for(i in 1:repeat_iter){
      ...
      m0.out = optim(pars_init,lik_fun_spline,splinef=splinef[ind],L2=sumL[,ncol(sumL)], control=list(fnscale=-1))
      Out[[i]]=m0.out
      LL[i]=m0.out$value
    }
    maxi=which.max(LL)
